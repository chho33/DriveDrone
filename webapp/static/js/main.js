jQuery(document).ready(function($){
	...
	if( cartWrapper.length > 0 ) {
		...
		var goToDelivery = $('.checkout.btn');
		var undo = cartWrapper.find('.undo');
		var undoTimeoutId;

        //go to delivery
        goToDelivery.on('click',function(event){
            event.preventDefault();
            var name_array = $('.product-details h3 a').toArray(),
                names = JSON.stringify(name_array.map(function(obj){ return obj.text})),
                img_array = $('.product-image img').toArray(),
                imgs = JSON.stringify(img_array.map(function(obj){ return obj.src})),
                price_array = $('.price').toArray(),
                prices = JSON.stringify(price_array.map(function(obj){ return $(obj).text()})),
                amount_array = $('.select :selected').toArray(),
                amounts = JSON.stringify(amount_array.map(function(obj) { return obj.text }));
            $('#delivery').html('<input type="hidden" name="names" value='+names+'><input type="hidden" name="imgs" value='+imgs+'><input type="hidden" name="prices" value='+prices+'><input type="hidden" name="amounts" value='+amounts+'>');
            $('#delivery').submit();
            return false;
        });
